...
If there is an intersection between these two lines, the order of $(y_{11}, y_{21)}$ and $(y_{12}, y_{22})$ will not be the same. To simplify, if $y_{1\ left} < y_{2\ left}$, then $y_{1\ right} > y_{2\ right}$ and, if $y_{1\ left} > y_{2\ left}$, then $y_{1\ right} < y_{2\ right}$. <br>
